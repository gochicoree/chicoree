...
const WEBHOOK_BODY: ApiParam[] = [
  ...
  { name: "format", in: "body", type: "json | none | slack | discord | teams | text", description: "Payload shape; default json. `none` sends no body at all — the request, its headers and authentication are the signal (deploy hooks such as the PaaS's, which read parameters from the URL and would misread the payload)." },
  { name: "method", in: "body", type: "GET | POST | PUT | PATCH", description: "For the json and none formats; chat formats always POST. GET never carries a body." },
