...
const WEBHOOK_BODY: ApiParam[] = [
  ...
  { name: "format", in: "body", type: "json | none | custom | slack | discord | teams | text", description: "Payload shape; default json. `none` sends no body at all — the request, its headers and authentication are the signal (deploy hooks that read parameters from the URL and would misread the payload). `custom` sends `payloadTemplate`." },
  { name: "payloadTemplate", in: "body", type: "string", description: "custom format only: a JSON object or array, up to 20000 characters, with {{placeholders}} — repository, organization, tag, digest, reference, registry, actor, timestamp, deliveryId, event.<path>; a string value that is exactly \"{{event}}\" embeds the whole event as JSON." },
  { name: "method", in: "body", type: "GET | POST | PUT | PATCH", description: "For the json, none and custom formats; chat formats always POST. GET never carries a body." },
...
export const API_CATALOG: ApiEndpoint[] = [
  ...
  },
  {
    ...
    description: "What counts against the caller's account limits: the organizations they own that have no limit of their own for that kind; with the month's traffic and the label administrators gave the account (a plan name, say).",
    access: "user",
    params: [MONTH_PARAM],
  ...
  },
  {
    ...
    description: "The account's limits row: caps on what the user owns, summed across their organizations that have no limit of their own. `configured` is false when there is no row (unlimited).",
    access: "admin",
    params: [USER_PARAM],
  ...
  },
  {
    ...
    description: "The organization's limits row. A limit set here governs the organization; the owners' account limits apply only to kinds it leaves unlimited. `configured` is false when there is no row.",
    access: "admin",
    params: [ORG_PARAM],
