              ...
              <div key={s.digest} className="flex flex-col gap-3 rounded-xl border border-line bg-card p-4">
                  ...
                  <div className="flex items-baseline gap-3">
                    ...
                    <span className="text-sm text-ink-2">
                      ...
                      {s.sbom.name && (
                        <>
                          {" "}
                          in <span className="font-mono text-[13px]">{s.sbom.name}</span>
                        </>
                      )}
                    </span>
                  </div>
                ...
                {s.sbom && s.sbom.components.length > 0 && (
                  <ul className="flex flex-wrap gap-1.5">
                    {s.sbom.components.map((c, i) => (
                      <li
                        key={`${c.name}-${i}`}
                        className="rounded-md border border-line bg-card-2 px-1.5 py-0.5 font-mono text-[11px] text-ink-2"
                        title={c.license ? `license ${c.license}` : undefined}
                      >
                        {c.name}
                        {c.version && <span className="text-ink-3">@{c.version}</span>}
                      </li>
                    ))}
                    {s.sbom.packageCount > s.sbom.components.length && (
                      <li className="px-1 py-0.5 text-[11px] text-ink-3">+{s.sbom.packageCount - s.sbom.components.length} more</li>
                    )}
                  </ul>
                )}
