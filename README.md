...
smaller installs may prefer Trivy or nothing.
...
machines without giving them database access. A worker is a separate,
small program — anything that speaks the protocol below — that needs
only the instance URL and the shared `SCAN_WORKER_TOKEN` (generated by
`scripts/deploy.sh`, or set it yourself in `.env`). Turn the hand-over on
under *Administration → Scanning → Offload scans to workers*
(`SCAN_WORKERS=true` as the default). From then on a push's scan becomes a
row in `scan_tasks`, and workers drive it over the **worker protocol**, all
`POST` with `Authorization: Bearer <SCAN_WORKER_TOKEN>` and a JSON body that
names the worker (`{ "worker": { "name", "hostname", "version",
"scannerVersion" } }`):
...
web container, so the option never stalls scanning. The Scanning page lists
the workers (name, Trivy version, running / completed / failed, last seen)
...
a PaaS deploy hook, which would misread a payload; *Custom JSON* sends a
body you write, with `{{repository}}`, `{{tag}}`, `{{digest}}`,
`{{reference}}`, `{{registry}}`, `{{organization}}`, `{{actor}}`,
`{{timestamp}}`, `{{deliveryId}}`, `{{event.<path>}}` filled in and a value
that is exactly `"{{event}}"` replaced by the whole event), extra headers,
...
- **Keyless CI**: a workflow exchanges the OIDC token its CI system issues
  ...
  identities*); `.github/actions/login` does it for GitHub Actions,
  `.github/actions/build-push` and `.github/actions/scan-gate` build on it
  and `release-images.yml` chains all three — see [GitHub Actions](#github-actions)
  and [API.md](API.md#keyless-ci-authentication).
...
Three composite actions and one shared workflow live under `.github/` so
that any repository — this one or yours — can build
images, push them to a Chicorée registry with their supply-chain evidence
and gate on the scan, without storing a registry secret:

| | |
| --- | --- |
| `.github/actions/login` | exchanges the job's OIDC token for a short-lived credential and runs `docker login` |
| `.github/actions/build-push` | builds with Buildx, pushes with an SPDX SBOM and SLSA provenance attached, signs with cosign and attests the SBOM |
| `.github/actions/scan-gate` | fails the job when the pushed image has vulnerabilities at or above a severity |
| `.github/workflows/release-images.yml` | the three chained into one reusable workflow: a list of images in, versioned, signed and scanned images out |

### From your own repository

1. **Reference them.** `uses: gochicoree/chicoree/.github/actions/login@main`
   (and `…/build-push@main`, `…/scan-gate@main`,
   `gochicoree/chicoree/.github/workflows/release-images.yml@main`). Pin
   a tag or commit instead of `main` when the pipeline must not change
   under you. While this repository is private, GitHub only lets
   repositories of the same organization use its actions; everyone else
   copies `.github/actions/` (and, if wanted, `release-images.yml`) into
   their own repository and references `./.github/actions/login` and so
   on — in a copied `release-images.yml`, change its three `uses:` lines to
   those local paths too. The actions need nothing but `bash`, `curl`,
   `jq` and Docker, all present on GitHub's hosted runners; the login
   exchange itself is plain HTTP, so GitLab CI or any other OIDC-issuing
   system can do the same with the curl lines in
   [API.md](API.md#keyless-ci-authentication).
2. **Trust the workflow once, on the registry.** Under *Organization →
   Service accounts → CI identities* add the issuer
   `https://token.actions.githubusercontent.com` and the subject GitHub
   ...
   `https://github.com/gochicoree/chicoree/.github/workflows/release-images.yml@*`
   when the shared workflow signs (the certificate names the workflow that
   ran the signing step, not the one that called it), or your workflow's
   own URI (`https://github.com/acme/api/.github/workflows/release.yml@refs/tags/*`)
   when it uses the `build-push` action directly. Prefer a key instead?
   Pass the PEM of a cosign private key and its password as secrets and
   add the public key under *Trusted signing keys*.
3. **Give the job an OIDC token.** `permissions: id-token: write` on the
   job (plus `contents: read` for the checkout) — the login action and
   keyless signing both need it. A registry without scanning needs
   `scan-gate: none` (or leave the gate out); one with a *Require
   signatures* policy needs step 2 before the first push, or the image is
   blocked until the identity is trusted.
4. **Call the shared workflow.** Your workflow only says what to build:
   ...
   jobs:
     images:
       uses: gochicoree/chicoree/.github/workflows/release-images.yml@main
       permissions:
         contents: read
         id-token: write
       with:
         ...
         images: |
           [
             { "name": "api", "context": "." },
             { "name": "worker", "context": "worker", "buildArgs": "VERSION={{version}}" }
           ]
   ```

   A tag `v1.2.3` on your repository becomes the tags `1.2.3`, `1.2`, `1`
   (from 1.0.0 on) and `latest`, plus `sha-<commit>`; a pre-release such
   as `v1.2.3-rc.1` gets only its own tag and leaves `latest` alone. Every
   image is built for `linux/amd64` and `linux/arm64` in one job (foreign
   platforms run under QEMU, so let the Dockerfile cross-compile as this
   repository's do), pushed with an SPDX SBOM and SLSA provenance as
   BuildKit attestation entries, signed with cosign — keyless with the
   job's identity, or with a key when the caller passes the `cosign-key`
   and `cosign-password` secrets — and the SBOM is attached once more as a
   signed in-toto attestation of each platform variant. The scan gate then
   waits for the scan and fails the job on findings at or above
   `scan-gate` (default `critical`; `high` is stricter, `none` skips it).
   Each image entry takes `name` (`org/repo`, or a bare name under
   `organization`), `context`, `file`, `target`, `platforms` and
   `buildArgs` (`KEY=value` lines; `{{version}}` and `{{sha}}` are
   replaced). Other inputs: `version` (instead of the tag), `platforms`,
   `latest`, `sha-tag`, `sign`, `attest` (`sbom`, `sbom,provenance` or
   `none`), `sbom`, `provenance`, `cosign-args`, `runs-on` and `cache`.
   The job summary lists what was pushed and links to the tag page. To run
   it by hand, add a `workflow_dispatch` trigger with a `version` input and
   pass it through, as this repository's `release.yml` does.
5. **Or use the actions on their own** when a workflow does more than
   release — a matrix, tests between build and push, several registries:
   ...
   `build-push` outputs `digest`, `ref` (`registry/org/repo@digest`),
   `tags`, `version` and `platforms`; the README next to each action lists
   every input, and `build-push`'s describes what each artifact looks like
   on the Attestations tab.
6. **Check what came out.** The tag page's Attestations tab shows the
   signature, the SBOM and the provenance with their verification state;
   from a terminal:

   ```sh
   cosign verify --certificate-oidc-issuer https://token.actions.githubusercontent.com \
     --certificate-identity-regexp '^https://github.com/gochicoree/chicoree/' \
     registry.example.com/acme/api:1.2.3
   cosign verify-attestation --type spdxjson --certificate-oidc-issuer https://token.actions.githubusercontent.com \
     --certificate-identity-regexp '^https://github.com/gochicoree/chicoree/' \
     registry.example.com/acme/api@sha256:<platform variant>
   docker buildx imagetools inspect registry.example.com/acme/api:1.2.3 --format '{{ json .SBOM }}'
   ```

### Releasing Chicorée

`.github/workflows/release.yml` calls the shared workflow for `web` and
`registryd`. Push a tag —

```sh
git tag v0.3.0 && git push origin v0.3.0
```

— or run *Release* by hand with a version, and both images land at
`chicor.ee/chicoree/web` and `chicor.ee/chicoree/registryd` for amd64 and
arm64, signed keylessly. The repository variables `CHICOREE_URL`,
`CHICOREE_ORG` and `SCAN_GATE` point a fork at its own registry or change
the gate; the secrets `COSIGN_PRIVATE_KEY` and `COSIGN_PASSWORD` switch to
key-based signing. `registryd` gets the version as its `VERSION` build
argument, which the [Health](#health) page shows. Deploying is not part
of it: a repository [webhook](#webhooks) on the registry tells
whatever runs the image that `latest` moved.
...
The design notes behind all of this are kept out of this repository.
