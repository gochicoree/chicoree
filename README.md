...
smaller installs may prefer Trivy or nothing. On the PaaS the compose file has
...
small program (`scan-worker`, shipped as its own image) that needs
...
the PaaS's deploy hook, which would misread a payload; *Custom JSON* sends a
